                    ...
                    <div class="ibox">
                        ...
                        <div class="ibox-content">
                            <table class="table table-bordered mg-b-0">
                                <p style="text-align: center; color: blue; font-size: 25px">Reservations</p>
                            </table>
<?php
$result = "";
	$pagenum = "";
	$per_page = 20;
		...
		$count = $con->query("SELECT * FROM reservation");
		...
		$reserve = $con->query("SELECT * FROM reservation LIMIT $start, $per_page");
		...
		if($reserve->num_rows) {
			...
			while($row = $reserve->fetch_assoc()) {
				...
				$result .=  "<tr>
								...
								<td>$time</td>
								<td>$suggestions</td>
								<td><a href='reservations.php?delete="."' onclick='return check();'><i class='pe-7s-close-circle'></i></a></td>
							</tr>";
	...
	if(isset($_GET['delete'])) {
		...
		if($delete != "") {
			...
			$sql = $con->query("DELETE FROM reservation WHERE reserve_id='".$delete."'");
...
<div class="wrapper">
    ...
    <div class="main-panel">
        <nav class="navbar navbar-default navbar-fixed" style="background: dodgerblue;
